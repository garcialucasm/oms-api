...
class ZoneController {
    ...
    async createZone(req, res) {
        ...
        try {
            ...
            if (err.name === 'ValidationError') {
                ...
                res.status(400).json({error: 'Duplicate zone code or zone name. Please use unique values.'});
            } else {
                res.status(500).json({ error: 'Error saving zone', details: err.message});
            }
        }
    }
    ...
    async updateZoneByCode(req, res) {
        console.log("PUT:/api/zones: " + req.params.cz + " - " + JSON.stringify(req.body));
        try {
            const {cz, name} = req.body;
            ...
            if (err.name === 'ValidationError') {
                ...
                res.status(400).json({ error: 'Duplicate zone code or zone name. Please use a unique values.' });
            } else {
